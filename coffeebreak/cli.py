...
@click.option('--venv', default=None, metavar='[PATH]', 
              help='Create/reuse virtual environment (default: .venv if no path given)')
@click.option('--conda', default=None, metavar='[NAME]',
...
def production(ctx, docker, domain, ssl_email, standalone, output_dir):
    ...
    try:
        ...
        if not ssl_email:
            ssl_email = click.prompt(f"Email for SSL certificates", default=f"admin@{domain}")
...
def _build_frontend(component_dir: Path, build_dir: Path, production: bool, verbose: bool) -> list:
    ...
    try:
        ...
        if package_json.exists():
            ...
            try:
                result = subprocess.run(["npm", "run", build_script], cwd=component_dir, 
    ...
    try:
        ...
        if (component_dir / "requirements.txt").exists() or (component_dir / "pyproject.toml").exists():
            ...
            if (component_dir / "requirements.txt").exists():
                subprocess.run([pip_cmd, "install", "-r", str(component_dir / "requirements.txt")], check=True)
        ...
        # Node.js project
            ...
            shutil.copytree(component_dir / "node_modules", build_dir / "node_modules", dirs_exist_ok=True)
...
@cli.command()
@click.option('--environment', '-e', default='production', help='Target environment (production, staging)')
...
def create(ctx, name, template, description, author, version):
    ...
    try:
        ...
        plugin_dir = plugin_env.create_plugin(
            name=name,
            template=template,
            **kwargs
        )
...
def build_plugin(ctx, name, output, include_native):
    ...
    try:
        ...
        pyz_path = plugin_env.build_plugin(
            ...
            output_dir=output,
            exclude_native=not include_native
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with tarfile.open(pub_package_path, 'w:gz') as tar:
                ...
                changelog_files = ['CHANGELOG.md', 'CHANGELOG.rst', 'CHANGELOG.txt', 'CHANGES.md']
...
def rotate(ctx, service, secret_type, force, backup):
    ...
    try:
        ...
        secret_manager = SecretManager(
            deployment_type=environment_type,
        ...
        if service:
            ...
            if not force:
                ...
                due_secrets = [name for name, info in rotation_status['schedules'].items() 
...
@secrets.command()
@click.option('--output', '-o', help='Output file path for backup (default: secrets-backup-{timestamp}.tar.gz)')
...
def backup(ctx, output, encrypt, password, service, include_metadata, compression):
    ...
    try:
        ...
        if encrypt:
            if not password:
                password = click.prompt("Enter password for backup encryption", hide_input=True, confirmation_prompt=True)
...
@click.option('--formats', default='markdown,html', help='Comma-separated formats (markdown, html, json)')
...
@click.pass_context
def generate(ctx, domain, output_dir, ssl_email, postgres_port, mongodb_port, 
             rabbitmq_port, redis, smtp, smtp_host, smtp_port, monitoring, backup):
...
def install(ctx, domain, ssl_email, user, install_dir, data_dir, log_dir):
    ...
    try:
        ...
        # Install standalone
        success = prod_env.install_standalone(
            domain=domain,
            ssl_email=ssl_email
        )
...
@click.option('--comprehensive', is_flag=True, help='Run comprehensive validation including health checks')
@click.pass_context
    ...
    try:
        ...
        if force:
            ...
            results = rotation_manager.emergency_rotation(
                ...
                reason="Manual force rotation via CLI"
            )
        else:
...
@click.option('--challenge', default='standalone', help='Challenge method (standalone, webroot, dns)')
...
@prod.command()
@click.option('--frequency', default='twice-daily', help='Renewal frequency (twice-daily, daily, weekly)')
@click.pass_context
