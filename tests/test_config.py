    ...
    def test_load_config_valid_yaml(self):
        ...
        valid_config = {
            'coffeebreak': {
                ...
                'version': '1.0.0',
                'repositories': [
                    {'name': 'core', 'url': 'https://github.com/test/core.git', 'path': './core', 'branch': 'main'}
                ]
            },
            'dependencies': {
                'services': {
        ...
        with patch('builtins.open', mock_open(read_data=yaml_content)):
            with patch.object(self.config_manager, 'get_config_path', return_value='test-config.yml'):
                with patch.object(self.config_manager, 'detect_environment', return_value=EnvironmentType.FULL_DEV):
    ...
    def test_validate_config_valid_main(self):
        ...
        valid_config = {
            'coffeebreak': {
                ...
                'version': '1.0.0',
                'repositories': [
                    {'name': 'core', 'url': 'https://github.com/PI-coffeeBreak/core.git', 'path': './core', 'branch': 'main'},
                    {'name': 'frontend', 'url': 'https://github.com/PI-coffeeBreak/admin-frontend.git', 'path': './frontend', 'branch': 'main'},
                    {'name': 'event-app', 'url': 'https://github.com/PI-coffeeBreak/event-app.git', 'path': './event-app', 'branch': 'main'}
                ]
            },
            'dependencies': {
                'services': {
                    'database': {'image': 'postgres:15', 'container_name': 'coffeebreak-db'},
                    'mongodb': {'image': 'mongo:6', 'container_name': 'coffeebreak-mongo'},
                    'rabbitmq': {'image': 'rabbitmq:3-management', 'container_name': 'coffeebreak-rabbitmq'},
                    'keycloak': {'image': 'quay.io/keycloak/keycloak:22', 'container_name': 'coffeebreak-keycloak'}
                }
            }
        }
